/*
 *  associated with 'DownloadServerDB.java' and 'DBPool.java'
 */

package com.ihateflyingbugs.vocaslide.AsyncTask;

import java.util.ArrayList;
import java.util.List;

import org.apache.http.NameValuePair;
import org.apache.http.message.BasicNameValuePair;
import org.json.JSONArray;
import org.json.JSONException;
import org.json.JSONObject;

import android.os.AsyncTask;
import android.util.Log;

import com.ihateflyingbugs.vocaslide.data.Config;
import com.ihateflyingbugs.vocaslide.data.DBPool;
import com.ihateflyingbugs.vocaslide.login.JSONParser;
public class Async_get_VERSION extends AsyncTask<String, String, String> {
	
	static final int VERSION_EXCEPTION= 0;
	static final int VERSION_SUCCESS= 1;
	
	JSONParser jParser = new JSONParser();
	
	protected String app_version;
	
	VocaCallback mCallback;
	
	public Async_get_VERSION(VocaCallback callback){
		Log.e("Async_get_FAQ constuctor", "Async_get_FAQ constuctor");
		mCallback = callback;
	}
	
	@Override
	protected void onPreExecute() {
		super.onPreExecute();
		
	}

	@Override
	protected String doInBackground(String... args) {
		
		List<NameValuePair> params = new ArrayList<NameValuePair>();
		params.add(new BasicNameValuePair("version",null));
		
		JSONObject json_version = jParser.makeHttpRequest("http://lnslab.com/vocaslide/Get_Version_Info.php", "GET", params);
				
		try {
			int success = json_version.getInt(Config.TAG_SUCCESS);	
			String message = json_version.getString("message");	
			String version = json_version.getString("version");
				
			if (success ==  VERSION_SUCCESS) {
				//MyInfoFragment.get_version(VERSION_SUCCESS, message, version);
				mCallback.Resonponse(json_version);
			}else{
				//MyInfoFragment.get_version(VERSION_EXCEPTION, message, null);
				mCallback.Exception();
			}
		} catch (JSONException e) {
			mCallback.Exception();
			e.printStackTrace();
		}
		return null;
	}


	protected void onPostExecute(String file_url) {

	}
	
	

}
